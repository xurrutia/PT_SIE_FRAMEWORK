...
class GUIBasePanel
{
  ...
  public GUIBasePanel(const shape &naviModule, const shape &mainModule, const shape &headerModule, const shape &infoModule, const shape &faceplateModule, const string &Screen)
  {
    ...
    DebugTN("HeaderModule", headerModule.ModuleName());
  ...
  public void Initialize()
  {
    ...
    classConnect(this, HideNavi, _hideNavi, Timer::TimerDone);
        ...
        // Dentro de public void Initialize()
    this.LoadHeader(); // Carga inicial

    // Nos conectamos al cambio de idioma del sistema para recargar el panel
    if (dpExists("_Connections.Language.Index"))
    {
      dpConnect("cbReloadHeaderOnLangChange", "_Connections.Language.Index");
    }
  }

  public void cbReloadHeaderOnLangChange(string dp, int lang)
  {
    this.LoadHeader();
  }
  ...
  public string GetInfoPanelName()
  {
    ...
    /**
   * @brief Función para cargar o recargar la cabecera (Header).
   * La sacamos fuera del constructor para poder llamarla al cambiar de idioma.
   */
  private void LoadHeader()
  {
    // Ajusta la ruta a tu panel de Ingeteam si es necesario
    string headerPath = "para/PanelTopology/templates/SIE/headerPanel_1024_768_SIE.pnl";

    if (shapeExists(this._headerModule))
    {
      RootPanelOnModule(headerPath, "Header", this._headerModule.ModuleName(), makeDynString());
    }
  }
